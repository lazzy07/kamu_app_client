...
class ChangePhoneScreen extends Component{
    ...
    login = async (pn) => {
        try{
            let oldP = this.props.navigation.state.params.phone.substring(1);
            oldP = "+94"+oldP;
            let res = await axios.post(FIREBASE_FUNCTION_URL+'/changephone', {
                phone: pn,
                oldPhone: oldP,
            ...
            if(res.status === 200){
                ...
                this.setState({
                    errorMessage: 'Cannot find username assigned to this phone number'
                })
                this.phoneInput.shake();
            }else if(res.status === 202){
                this.setState({
                    errorMessage: 'Account with same phone number already found'
                })
                this.phoneInput.shake();
            }else{
                this.setState({
